...
void delay (unsigned int howLong){
 ...
 int writecmd(char *command) {
        ...
        printf("\nDigole CMD: %s ",command);
		for (i=0;i<strlen(tmp);i++) {
			printf("\nCharacter is: %c",tmp[i]);
...
void setRBGColor(uint8_t r, uint8_t g, uint8_t b){
    writecmd("ESC");
    writebyte(r);
    writebyte(g);
    writebyte(b);
...
// write 3 colors bytes for pixel image to the display
void drawBitmap256K(uint8_t x, uint8_t y, uint8_t w, uint8_t h, const uint8_t *bitmap,uint8_t c ) {
	int j;
    writecmd("EDIM3");
    writebyte(x);
    writebyte(y);
...
int main (int argc, char* argv[]) {
    ...
    } else if (strcmp(digoleCommand, "setRBGColor") == 0) {
        setRBGColor((uint8_t)atoi(argv[2]), (uint8_t)atoi(argv[3]), (uint8_t)atoi(argv[4]));
    } else if (strcmp(digoleCommand, "directByte") == 0) {
        writebyte((uint8_t)atoi(argv[2]));
    } else if (strcmp(digoleCommand, "myimage") == 0) {
        drawBitmap256K(0, 0, 160, 128, &imageData,0);
    }
    printf("\n");
	return 0;
}